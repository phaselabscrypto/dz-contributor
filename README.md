...
| `GET /api/methodology` | Machine-readable formulas + sources (no page; the Methodology page was removed) |
...
Every response is labelled with the `method` used —
...
User picks a contributor → modifies links + demand → the page submits an
async job to `/api/shapley/jobs` and polls it; the response includes
`before`/`after` share + 2Z projections + per-contributor delta.
...
the Shapley, link-value, and diff cards render nothing or a 503 state
...
- **Registry program ID + IDL** → flips `/api/onchain/topology` and
  `/api/onchain/validators` from stub to live. Contributor rewards are
  already read live from the DZ ledger.
