...
function link(
  ...
  return {
    ...
    link_health: "ok",
    status: "activated",
    code: side_a_pk + side_z_pk,
  };
}

function snapshot(
  serviceability: RawSnapshot["fetch_data"]["dz_serviceability"],
): RawSnapshot {
  return {
    dz_epoch: 211,
    solana_epoch: 900,
...
const shape = extractDiffShape(
  snapshot({
    locations: { LocA: location("AMS"), LocB: location("FRA") },
    contributors: {
      ContribOne: contributor("one"),
      ContribTwo: contributor("two"),
    },
    devices: {
