...
7. [Limits](#limits)
8. [What-if simulation](#what-if-simulation)
9. [Correctness pinning](#correctness-pinning)
...
|---|---|---|---|
| Sync `/link-estimate` focus cap | `SYNC_MAX_FOCUS_LINKS` = 12 | `link_estimate` handler, `services/shapley-rs/src/routes.rs` | 12 links is 13 players, `2^13` = 8,192 coalitions. Production logs measure about 218 ms per coalition on 4 solver threads, so 12 links takes about 30 minutes. Above 12 the sync path returns 422 and points the caller to `POST /jobs/link-estimate`. |
| Sweep / async focus cap | `SWEEP_MAX_FOCUS_LINKS` = 19 | `link_estimate_start` (422) and `run_sweep` (skip), `services/shapley-rs/src/routes.rs` and `src/worker.rs` | 19 links is 20 players, `2^20` ≈ 1.05M coalitions, about 63 hours at 218 ms/coalition. 18 links (`2^19`) is about 32 hours. Both outlast an epoch, so `POST /jobs/link-estimate` returns 422 and the sweep skips the operator instead of enqueueing a job that cannot finish. |
...
The pipeline is pinned at two layers: the Rust engine wrapper (per-coalition and full-epoch regression) and the TS input builder / live pipeline. The fixtures:
