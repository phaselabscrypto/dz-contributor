...
| `/validators` | `app/validators/page.tsx` | Publishing validators: stake-weighted SOL projection, plus an earnings estimate for any vote account |
...
| `/simulate` | `app/simulate/page.tsx` | Forecast tool: add or remove links, modify demand, see the Shapley delta |
...
`components/economics/live-baseline-shapley.tsx` shows the Shapley `method` label (see [Method labels](#method-labels)). Its `methodLabel` helper matches the `lp-` prefix and prints "Canonical LP", so a service-side rename inside that family does not break the UI.
...
- **On-chain**: `onchain/{contributors,rewards,contributor-rewards}` read the DZ ledger live and return a `502` on failure; `onchain/{topology,validators}` pre-flight-check configuration and return `503` with a stable `{ ready: false, reason }` shape until `ONCHAIN_ENABLED` / `DZ_REGISTRY_PROGRAM_ID` are set.
- **Meta**: `methodology` (machine-readable formula/source manifest, JSON only) and `vitals` (Web Vitals sink; always `204`, logs only outside production).
...
- **Input builders**: `canonical-input-builder.ts` (bit-comparable to the Foundation reference; the only builder on the published-baseline path), `shapley-input-modifier.ts`, `link-edits.ts`, `demand-overrides.ts` (apply simulate edits).
...
**Live, bit-verified readers.** `dz-rewards-record.ts`, `rewards.ts`, and `contributor-directory.ts` read contributor-rewards records and the contributor directory from the DZ ledger (`DZ_LEDGER_RPC_URL`). `vote-stake.ts` resolves any vote account or node identity to its activated stake over Solana RPC for `/api/validators/stake`. `client.ts` is the JSON-RPC client behind that lookup and the measured epoch cadence in `lib/utils/epoch-rate.ts`.

**Stubs.** `decoders.ts`, `topology.ts`, and `validators.ts` wait for the DoubleZero program IDL.

`program-ids.ts` defines `SOLANA_RPC_URL` (defaults to `https://api.mainnet-beta.solana.com`), `DZ_REGISTRY_PROGRAM_ID`, `DZ_REWARDS_PROGRAM_ID`, and the `ONCHAIN_ENABLED` toggle; until the IDL is checked in at `lib/onchain/idl/` and the registry swapped from `stubRegistry` to `anchorRegistry`, `/api/onchain/topology` and `/api/onchain/validators` return `503`. The activation checklist lives in `lib/onchain/README.md`.

### Rust service (`services/shapley-rs/`)

An axum + tokio + rayon HTTP wrapper around `network-shapley-rs`. Its endpoints fall into five groups:

| Group | Endpoints |
|---|---|
| Sync solvers | `POST /shapley`, `POST /simulate`, `POST /link-estimate` |
| Async jobs | `POST /jobs/simulate`, `POST /jobs/link-estimate`, `POST /jobs/link-estimate/by-tag`, `GET`/`DELETE /jobs/{id}` |
| Cache-only read | `GET /shapley/baseline?tag=` |
| Publication (ingest-gated, cron-driven) | `POST /precompute/link-estimates`, `POST /precompute/baseline`, `PUT /diff/shape/{epoch}`, and the probes `GET /precompute/link-estimates/status` and `GET /diff/missing` |
| Diff reads | `GET /diff`, `GET /diff/contributor/{code}` |

One S3-compatible bucket holds all of it: solver results by input hash, epoch aliases and sweep markers under `shapley/v3/publication/v1/`, and diff shapes under `diff/v1/`. Each result is computed once. Its Dockerfile builds an OpenShift-compatible image that runs as a non-root user with gid=0 and `chmod g=u`. [shapley-service.md](./shapley-service.md) covers the endpoints, the Redis Streams queue, the result cache, and bearer auth. The algorithm is in [shapley-pipeline.md](./shapley-pipeline.md).

## Request flows

### a. `/simulate` (async what-if job)

The simulate page holds the selected contributor + epoch in the URL via `nuqs` (`app/simulate/page.tsx`), resolves the latest epoch with `useEpochs()`, and loads the snapshot with `useSnapshot(epoch)`. Edits (add/remove links, demand overrides) are local React state. On **Calculate**, `components/simulator/simulate-tab.tsx` drives the **async job API**, because a full re-solve can take minutes:
...
In-memory caches are **per Vercel function instance**, so a scale-out fleet holds N independent copies. Snapshots are immutable for completed epochs, which is why they (and the diff/shapley routes derived from them) can cache aggressively at every layer.
...
The publisher feed treats Foundation exports as authoritative and malbec as a best-effort enrichment overlay (`app/api/publishers/route.ts`). The earliest published snapshot epoch is `MIN_DZ_EPOCH = 48` (`lib/constants/config.ts`); no upper bound is pinned, so the S3 `404` rejects epochs that do not exist yet.
...
|---|---|---|
| `lp-per-city-stake-weighted-exact` | Canonical Rust solver. The service stamps this on every `/shapley`, `/simulate`, and async-job result | `services/shapley-rs/src/routes.rs` |
...
The limiter is advisory by design. When no trusted IP can be identified the request proceeds rather than sharing one bucket across unknown callers, and the bucket map is bounded so a header-spoofing flood cannot OOM the instance. It throttles pathological retries from a single client; it is not a global SLA.

**Service-to-service auth.** The Rust service uses fail-closed bearer auth. The Next.js routes attach `SHAPLEY_API_TOKEN` as `Authorization: Bearer …`, which never reaches clients. See [shapley-service.md](./shapley-service.md).
...
The `/api/health` aggregator (`app/api/health/route.ts`) is hardened too. It returns only hostnames and coarse error categories, so probing it cannot leak upstream routing or credentials.
...
The fire holds a 270 s work budget and gives 90 s of it to historical repair. Repair takes up to two shape gaps per fire, newest first, rotating every six hours so no gap starves. Passing `?epoch=N` with the bearer token backfills one epoch by hand.
