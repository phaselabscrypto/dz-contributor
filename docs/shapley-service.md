...
| Sync `/link-estimate` focus cap (`SYNC_MAX_FOCUS_LINKS`) | 12 | `link_estimate` handler. 8,192 coalitions, measured at 1,783 s. Above it the handler returns 422 pointing at `/jobs/link-estimate`, rather than burning the solver pool until the request timeout cancels it |
| Sweep / async focus cap (`SWEEP_MAX_FOCUS_LINKS`) | 19 | `run_sweep` in `src/worker.rs` (skip) and `link_estimate_start` in `src/routes.rs` (422). 1,048,576 coalitions, about 63.4 h. Operators above it appear in the sweep summary's `skipped` list rather than being enqueued as a job that will not finish |
