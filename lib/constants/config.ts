...
// the same one on every render. Cleared per server boot / page load, and
// capped because the codes reaching it come from public request paths.
const MAX_WARNED_CONTRIBUTORS = 500;
...
export function getContributorDisplayName(code: string): string {
  ...
  if (code && !warnedUnknownContributors.has(code)) {
    if (warnedUnknownContributors.size >= MAX_WARNED_CONTRIBUTORS) {
      warnedUnknownContributors.clear();
    }
